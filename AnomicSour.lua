...
notify("Anomic Sour", "Scripts made by H3LLO and Rhot1c - Forum: F A Z E D")
...
-- Others Page
local Other = Main:addPage("Other Players", 5012544693)
local OtherSection = Other:addSection("Other Player")
local players = game:GetService("Players")
...
OtherSection:addButton("Get Target Backpack", function()
    for i,v in pairs(game:service'Players':GetPlayers()) do
        if v.Name:match(targetName) then
...
UISection:addKeybind("GUI Keybind", Enum.KeyCode.Insert, function()    
    Main:toggle()
    end, function()    
end)
